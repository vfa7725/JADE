...
public class MainWin extends JFrame {
    ...
    public MainWin(String title) {
        ...
        JMenuItem mDonEd = new JMenuItem("Donut");
        JMenuItem mJavEd = new JMenuItem("Java");
        JMenu mReport = new JMenu("Report");
        JMenuItem mProfit = new JMenuItem("Profit and Loss");
        JMenuItem mServerL = new JMenuItem("Server Report");
        ...
        mJavEd.addActionListener(event -> onCreateJavaClick2());
        mDonEd.addActionListener(event -> onCreateDonutClick2());
        mProfit.addActionListener(event -> onProfitAndLossClick());
        mServerL.addActionListener(event -> onServerReportClick());
        ...
        bDonutEd = new JButton((new ImageIcon("gui/resources/list_customers.png")));
        bDonutEd.setActionCommand("Edit Donut");
        bDonutEd.setToolTipText("Edit Donut");
        toolbar.add(bDonutEd);
        bDonutEd.addActionListener(event -> onCreateDonutClick2());

        bJavaEd = new JButton((new ImageIcon("gui/resources/list_customers.png")));
        bJavaEd.setActionCommand("Edit Java");
        bJavaEd.setToolTipText("Edit Java");
        toolbar.add(bJavaEd);
        bJavaEd.addActionListener(event -> onCreateJavaClick2());

        bPal = new JButton((new ImageIcon("gui/resources/list_customers.png")));
        bPal.setActionCommand("Profit and Loss");
        bPal.setToolTipText("Profit and Loss");
        toolbar.add(bPal);
        bPal.addActionListener(event -> onProfitAndLossClick());

        bServe = new JButton((new ImageIcon("gui/resources/list_customers.png")));
        bServe.setActionCommand("Server Report");
        bServe.setToolTipText("Server Report");
        toolbar.add(bServe);
        bServe.addActionListener(event -> onServerReportClick());
        ...
        try {
            ...
            for (Object o : shotPanel.getComponents()) {
                if (!(o instanceof JComboBox))
                    continue; // verify cast will work, then
    ...
    protected int onJavaNum() {
        String[] array = new String[java1.size()];
        for (int i = 0; i < array.length; i++) {
            array[i] = (java1.get(i).toString());
        }
        JLabel lDarkness = new JLabel("<HTML><BR/>Java</HTML>");
    ...
    protected void onCreateJavaClick2() { // Create a new Java product
        int n = onJavaNum();
        try {
            // Name of Java
            JLabel lName = new JLabel("Name");
            JTextField dName = new JTextField(20);
            dName.setText(java1.get(n).getName());

            // Price
            JLabel lPrice = new JLabel("<HTML><BR/>Price</HTML>");
            JSpinner dPrice = new JSpinner(new SpinnerNumberModel(java1.get(n).getPrice(), 0.0, 99.99, 0.05));

            // Cost
            JLabel lCost = new JLabel("<HTML><BR/>Cost</HTML>");
            JSpinner dCost = new JSpinner(new SpinnerNumberModel(java1.get(n).getCost(), 0.0, 99.99, 0.05));

            // Darkness
            JLabel lDarkness = new JLabel("<HTML><BR/>Darkness</HTML>");
            JComboBox<Object> dDarkness = new JComboBox<>(Darkness.values());
            dDarkness.setSelectedItem(java1.get(n).getDarkness());
    ...
    protected void onCreateDonutClick() { // Create a new Java product
        try {
            ...
            if (button == JOptionPane.OK_OPTION) {
                ...
                Donut d1 = new Donut(name, price, cost,
    ...
    protected int onDonutNum() {
        String[] array = new String[donut.size()];
        for (int i = 0; i < array.length; i++) {
            array[i] = (donut.get(i).toString());
        }
        JLabel lDarkness = new JLabel("<HTML><BR/>Donut</HTML>");
        JComboBox comboBox = new JComboBox(array);
    ...
    protected void onCreateDonutClick2() { // Create a new Java product
        int n = onDonutNum();
        try {
            // Name of Donut
            JLabel lName = new JLabel("Name");
            JTextField dName = new JTextField(20);
            dName.setText(donut.get(n).getName());

            // Price
            JLabel lPrice = new JLabel("<HTML><BR/>Price</HTML>");
            JSpinner dPrice = new JSpinner(new SpinnerNumberModel(donut.get(n).getPrice(), 0.0, 99.99, 0.05));

            // Cost
            JLabel lCost = new JLabel("<HTML><BR/>Cost</HTML>");
            JSpinner dCost = new JSpinner(new SpinnerNumberModel(donut.get(n).getCost(), 0.0, 99.99, 0.05));

            // Frosting
            JLabel lFrosting = new JLabel("<HTML><BR/>Frosting</HTML>");
            JComboBox<Object> dFrosting = new JComboBox<>(Frosting.values());
            dFrosting.setSelectedItem(donut.get(n).getFrosting());

            // Filling
            JLabel lFilling = new JLabel("<HTML><BR/>Filling</HTML>");
            JComboBox<Object> dFilling = new JComboBox<>(Filling.values());
            dFilling.setSelectedItem(donut.get(n).getFilling());
            ...
            JComboBox<Object> dSprinkles = new JComboBox<>(options);
            int term = 0;
            if (donut.get(n).isSprinkles()) {
                term = 1;
            }
            dSprinkles.setSelectedIndex(term);
    ...
    protected void onProfitAndLossClick() {
        JFrame f1 = new JFrame();
        // f1.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
        JPanel panel = new JPanel();
        panel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "ODI Rankings",
                TitledBorder.CENTER, TitledBorder.TOP));
        String[][] rec = new String[products1.size()][4];
        String[] header = { "Product", "Income", "Cost", "Profit" };
        for (int i = 0; i < products1.size(); i++) {
            for (int j = 0; j < 4; j++) {
                if (j == 0) {
                    rec[i][j] = products1.get(i).name();
                } else if (j == 1) {

                    rec[i][j] = products1.get(i).price() + "";
                } else if (j == 2) {

                    rec[i][j] = products1.get(i).cost() + "";
                } else if (j == 3) {
                    double prof = products1.get(i).price() - products1.get(i).cost();
    ...
    protected void onServerReportClick() {
        ...
        String[][] rec = new String[server1.size()][4];
        String[] header = { "Name", "Phone", "SSN", "Orders" };
        for (int i = 0; i < server1.size(); i++) {
            for (int j = 0; j < 4; j++) {
                if (j == 0) {
                    rec[i][j] = server1.get(i).name();
                } else if (j == 1) {

                    rec[i][j] = server1.get(i).getPhone();
                } else if (j == 2) {

                    rec[i][j] = server1.get(i).getSsn();
                } else if (j == 3) {
                    double prof = store.numberOfOrders() / server1.size();
        ...
        while (true) {
            try {
                ...
                if (button == JOptionPane.OK_OPTION) {
                    ...
                    Server s1 = new Server(name, phone, ssn);
                    store.addPerson(s1);
                    server1.add(s1);
                    updateDisplay(Display.PEOPLE);
                }
    ...
    private void updateDisplay(Display display) {
        ...
        if (display == Display.PRODUCTS)
            s = store.toString();
        if (display == Display.PEOPLE)
            s = store.peopleToString();
        if (display == Display.ORDERS)
            s = store.ordersToString();
    ...
    private JButton bDonutEd;
    private JButton bJavaEd;
    private JButton bPal;
    private JButton bServe;

    private ArrayList<Java> java1 = new ArrayList<Java>();
    private ArrayList<Donut> donut = new ArrayList<Donut>();
    private ArrayList<Product> products1 = new ArrayList<Product>();
    private ArrayList<Server> server1 = new ArrayList<Server>();
